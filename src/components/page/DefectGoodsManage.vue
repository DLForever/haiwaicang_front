	...
	<div class="table">
		...
		<div class="container">
			...
			<el-table :data="data" border style="width: 100%" model="form" ref="multipleTable" @selection-change="handleSelectionChange">
				...
				<el-table-column fixed prop="fnsku" label="Fnsku">
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				this.$axios.get('/cargos/search_by_fnsku?page=' + this.cur_page + '&defect=true' + '&query=' + this.search_fnsku.trim(), {
					headers: {
...
//				this.dynamicTags = []
			...
			handleDetial(index, row) {
				this.$axios.get('/transfer_records?cargo_id=' + row.id, {
					headers: {
						'Authorization': localStorage.getItem('token')
					},
				}).then((res) => {
