	...
	<div>
		<div class="crumbs">
			<el-breadcrumb separator="/">
				<el-breadcrumb-item><i class="el-icon-date"></i> 出库单管理(入库即出)</el-breadcrumb-item>
		...
		<div class="container">
			...
			<el-form :rules="rules" label-width="50px">
				<el-form-item label="批次" required>
					<el-select @change="assignmentFnsku" v-model="select_batch" placeholder="选择批次" class="handle-select mr10">
...
//							this.options = res.data.data
			...
			getBatchInbound() {
				this.$axios.get('/batch_store_ins?is_quick=1&page=' + this.batch_page, {
					headers: {
						'Authorization': localStorage.getItem('token')
					}
				}).then((res) => {
			...
			onSubmit() {
				if(this.select_batch == '') {
					this.$message.info("请选择批次")
					return
				}
				if(this.radio == undefined) {
					this.$message.info("请选择是否混装")
					return
				}
				console.log('lllll')
				...
				formData.append('batch_store_in_id', this.batch_store_in_id)
				let params = {
					fnsku: fnsku,
					dst_fnsku: new_fnsku,
					sum: plan_sum,
					...
					is_mix: this.radio,
					remark: this.remark,
					batch_store_in_id: this.batch_store_in_id
				}
				this.submitDisable = true
				// setTimeout(() => {
				// 	this.submitDisable = false
				// }, 10000)
				this.$axios.post('/outbound_orders/out', formData, {
					headers: {
						'Authorization': localStorage.getItem('token')
					}
				}).then((res) => {
					...
					if(res.data.code == 200) {
						...
						this.submitDisable = false
						this.$router.push('outboundmanagequick');
    ...
    //     		}
			...
			assignmentFnsku() {
				this.options = []
				this.batch_store_in_id = this.select_batch
				// this.select_batch.batch_store_in_infos.forEach((data) => {
				// 	this.options.push(data.fnsku)
				// })
				let index = this.batch_options.findIndex((data) => data.id == this.select_batch)
				this.batch_options[index].batch_store_in_infos.forEach((data) => {
					this.options.push(data.fnsku)
				})
			}
		},
