	...
	<div>
		<div class="crumbs">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>出库统计</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		<div class="container">
			...
			<el-table :data="data" show-summary :summary-method="getSummaries" border style="width: 100%" model="form" ref="multipleTable" @selection-change="handleSelectionChange">
				...
				<el-table-column prop="barcode" label="出库单号"></el-table-column>
				<el-table-column prop="fnsku" label="fnsku"></el-table-column>
				<el-table-column prop="sum" label="出库数量"></el-table-column>
				...
				this.$axios.get('/admin/outbound_orders/statistics?page=' + this.cur_page + '&fnsku=' + this.search_fnsku + '&date_begin=' + date_begin_temp +'&date_end=' + date_end_temp, {
					...
					if(res.data.code == 200) {
						res.data.data.forEach((data) => {
							data.barcode = 'hwc_' + data.outbound_order_id
							// data.sum = 0
							// data.lock_sum = 0
